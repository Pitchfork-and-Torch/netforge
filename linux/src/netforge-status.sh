...
JSON=false; HTML_PATH=""; SKIP_DNS=false; SAMPLE=false; CONFIG_FILE="${REPO_ROOT}/config/defaults.conf"
while [[ $# -gt 0 ]]; do case "$1" in --json) JSON=true; shift;; --html) HTML_PATH="$2"; shift 2;; --skip-dns-probe) SKIP_DNS=true; shift;; --sample) SAMPLE=true; shift;; --config) CONFIG_FILE="$2"; shift 2;; *) shift;; esac; done
netforge_load_config "$CONFIG_FILE"
VERSION=$(tr -d '\r' <"$REPO_ROOT/VERSION" 2>/dev/null || echo unknown)
TS=$(date -Iseconds 2>/dev/null || date -u +%Y-%m-%dT%H:%M:%SZ)
if [[ "$SAMPLE" == true ]]; then
  printf '{"kind":"netforge-sample-receipt","tool":"NetForge","version":"%s","sample":true,"beats":["Prefer Ethernet when a wired adapter is up","Resilient DNS (DoH when available)","TCP tuning for lossy last-mile","Fail closed on captive portal"],"changed":[],"note":"SAMPLE. No settings were changed."}\n' "$VERSION"
  exit 0
fi
if [[ "$JSON" == true ]]; then
