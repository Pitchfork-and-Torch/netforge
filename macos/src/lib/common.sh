...
# Classify a macOS networksetup service name. VPN first so "VPN Ethernet" does not look wired.
service_type() {
  local svc="$1"
  if [[ "$svc" =~ [Vv][Pp][Nn]|[Ww]ire[Gg]uard|[Tt]unnel|[Tt]ailscale|[Zz]scaler|[Aa]ny[Cc]onnect|[Gg]lobal[Pp]rotect|[Zz]ero[Tt]ier|[Mm]ullvad|[Pp]roton|[Oo]pen[Vv][Pp][Nn]|[Nn]ord ]]; then
    echo vpn
  elif [[ "$svc" =~ [Ww][Ii][-[:space:]]?[Ff][Ii] ]]; then
    echo wifi
...
netforge_load_config() {
  ...
  local user_dir="${HOME}/Library/Application Support/${APP_NAME}"
  DATA_DIR="$(netforge_pick_data_dir /var/log/netforge "$user_dir")"
