...
apply_network_services() {
  ...
  if [[ "$has_eth" == true && "$has_wifi" == true ]]; then
    ...
    local -a order_svcs=("${eth_svcs[@]}" "${wifi_svcs[@]}" "${other_svcs[@]}")
    if [[ "$DRY_RUN" == true ]]; then plan "service order Ethernet before Wi-Fi"
    else networksetup -ordernetworkservices "${order_svcs[@]}" 2>/dev/null || true; netforge_log "Service order: Ethernet before Wi-Fi"; fi
  fi
...
apply_awdl() {
  # Only take AWDL down when DISABLE_AWDL=true and a wired Ethernet service is present.
  command -v ifconfig >/dev/null 2>&1 || return 0
  local has_eth=false svc
  if command -v networksetup >/dev/null 2>&1; then
    while IFS= read -r svc; do
      [[ -z "$svc" || "$svc" == *"*"* ]] && continue
      [[ "$(service_type "$svc")" == ethernet ]] && has_eth=true
    done < <(networksetup -listallnetworkservices 2>/dev/null | tail -n +2)
  fi
  if ! netforge_should_disable_awdl "$has_eth"; then
    [[ "$DRY_RUN" == true ]] && plan "keep AWDL"
    return 0
  fi
  if [[ "$DRY_RUN" == true ]]; then plan "disable AWDL on wired (DISABLE_AWDL=true)"; return 0; fi
