...
[CmdletBinding()]
param(
    [string]$RepoUrl = 'https://github.com/Pitchfork-and-Torch/netforge.git',
...
$platformRoot = Join-Path $InstallDir $Platform
$installer = Join-Path $platformRoot 'src\Install-NetworkAuto.ps1'
if (-not (Test-Path $installer)) {
    throw "Installer not found after clone: $installer"
}

& $installer -RepoRoot $platformRoot
