...
. (Join-Path $PSScriptRoot 'lib\NetForge.Adapters.ps1')
...
function Get-ActiveNetAdaptersSafe {
    ...
    foreach ($a in @(Get-NetAdapter -ErrorAction SilentlyContinue | Where-Object { $_.Status -eq 'Up' })) {
        ...
        if (Test-NetForgeShouldSkipVpn -Adapter $a -RespectVpn $respectVpn) {
            Write-NetForgeLog "RespectVpn: skip $($a.Name)"
            continue
        }
        if (-not (Test-NetForgeWirelessAdapter $a) -and -not (Test-NetForgeEthernetAdapter $a)) {
...
try {
    ...
    $eth = @($adapters | Where-Object { Test-NetForgeEthernetAdapter $_ })
    $wifi = @($adapters | Where-Object { Test-NetForgeWirelessAdapter $_ })
